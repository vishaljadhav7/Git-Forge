...
import { loadRepoSchemaType , createQuestionSchemaType} from "../schema/repo.schema";
  ...
  loadRepoToQuery = async (
    ...
    try {
      ...
      logger.info(`Starting repo load process for project: ${projectData.projectId}`);
       ...
       await this.repoService.loadGitRepo(projectData.projectId, projectData?.branchName, req.user.userId);
